...
class MiraBlobTracker(object):
    ...
    def __init__(self):
        ...
        rospy.Subscriber(joint_states_topic_name, JointState,
                         self.mira_joints_callback)
        mira_joints_data = None
        while mira_joints_data is None:
            try:
                mira_joints_data = rospy.wait_for_message(
                    joint_states_topic_name, JointState, timeout=5)
            except:
                rospy.logwarn("Time out " + str(joint_states_topic_name))
                pass

        self.mira_joint_dictionary = dict(
            zip(mira_joints_data.name, mira_joints_data.position))
        print self.mira_joint_dictionary
        rospy.Subscriber('/mira/commands/velocity',
                         Twist, self.blob_info_callback)

    def blob_info_callback(self, msg):
        rospy.loginfo("Blob info Detected==> about x:" +
                      str(msg.angular.x)+"about y:"+str(msg.angular.y)+"about z:"+str(msg.angular.z))
        r_value = msg.angular.x
        up_value = msg.angular.y
        turn_value = msg.angular.z
        roll_actual_pos = self.mira_joint_dictionary.get("roll_joint")
        next_value_roll = roll_actual_pos + r_value
        pitch_actual_pos = self.mira_joint_dictionary.get("pitch_joint")
        next_value_pitch = pitch_actual_pos + up_value
        yaw_actual_pos = self.mira_joint_dictionary.get("yaw_joint")
        next_value_yaw = yaw_actual_pos + turn_value
        rospy.loginfo("Move Head to Blob==>" +
                      str(next_value_roll)+", "+str(next_value_pitch)+", "+str(next_value_yaw))
        # self.move_mira_yaw_joint(position=next_value)
        self.move_mira_all_joints(
            roll=next_value_roll, pitch=next_value_pitch, yaw=next_value_yaw)
    ...
    def mira_check_joint_value(self, joint_name, value, error=0.1):
        ...
        :return:
        """
        similar = self.mira_joint_dictionary.get(joint_name) >= (
            value - error) and self.mira_joint_dictionary.get(joint_name) <= (value + error)
    ...
    def mira_movement_look(self, roll, pitch, yaw):
        ...
        while not (similar_roll and similar_pitch and similar_yaw):
            self.move_mira_all_joints(
                position_roll, position_pitch, position_yaw)
            similar_roll = self.mira_check_continuous_joint_value(
                joint_name="roll_joint", value=position_roll)
            similar_pitch = self.mira_check_continuous_joint_value(
                joint_name="pitch_joint", value=position_pitch)
            similar_yaw = self.mira_check_continuous_joint_value(
                joint_name="yaw_joint", value=position_yaw)
            rate.sleep()
